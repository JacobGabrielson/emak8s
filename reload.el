...
(defun reload-k8s ()
  ...
  (dolist (buf (buffer-list))
    (when (string-prefix-p "*k8s:" (buffer-name buf))
      (kill-buffer buf)))
  (dolist (feat '(k8s-pods k8s k8s-fs k8s-exec k8s-watch k8s-api k8s-config))
    (when (featurep feat) (unload-feature feat t)))
  ;; Byte-compile everything
  (dolist (file '("k8s-config.el" "k8s-api.el" "k8s-watch.el" "k8s-exec.el"
                  "k8s-fs.el" "k8s.el" "k8s-pods.el"))
